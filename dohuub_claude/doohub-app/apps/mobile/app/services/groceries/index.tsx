...
export default function GroceriesScreen() {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      setLoading(true);
      const stores = await getStoresByCategory('GROCERIES');
      const mapped = stores.map((store: any, idx: number) => {
        const v = store.Vendor;
        return {
          id: store.id,
          name: store.name || v?.businessName || 'Store',
          rating: v?.rating ?? 0,
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>
                {error || 'No grocery stores available'}
              </Text>
              {error && (
